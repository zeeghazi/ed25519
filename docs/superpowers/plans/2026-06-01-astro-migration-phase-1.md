...
@theme {
  ...
  --shadow-e2: 0px 1px 1px #00000005, 0px 2px 2px #0000000a, inset 0 0 0 1px #00000014;
  --shadow-e3: 0px 2px 2px #0000000a, 0px 8px 8px -8px #0000000a, inset 0 0 0 1px #00000014;
  --shadow-e4: 0px 2px 2px #0000000a, 0px 8px 16px -4px #0000000a, inset 0 0 0 1px #00000014;
  --shadow-e5: 0px 1px 1px #00000005, 0px 8px 16px -4px #0000000a, 0px 24px 32px -8px #0000000f, inset 0 0 0 1px #00000014;
}
...
@utility mesh-gradient {
  ...
  background-image:
    radial-gradient(at 20% 20%, var(--color-grad-develop-end) 0px, transparent 50%),
    radial-gradient(at 80% 0%, var(--color-grad-preview-start) 0px, transparent 50%),
    radial-gradient(at 70% 50%, var(--color-grad-preview-end) 0px, transparent 50%),
    radial-gradient(at 0% 70%, var(--color-grad-ship-start) 0px, transparent 50%),
    radial-gradient(at 90% 80%, var(--color-grad-ship-end) 0px, transparent 50%);
}
```
...
export function isHex(value: string, expectedLength?: number): boolean {
  ...
  if (expectedLength !== undefined && clean.length !== expectedLength) return false;
  return true;
}
```
...
```ts
import { describe, it, expect } from 'vitest';
import { generateKeypair, signMessage, verifySignature } from '../src/lib/ed25519';
...
describe('ed25519', () => {
  ...
  it('signs and verifies a round-trip', async () => {
    ...
    expect(await verifySignature(sig, 'hello noble', publicKeyHex)).toBe(true);
  });
  ...
  it('rejects a public key that is not 64 hex chars', async () => {
    await expect(verifySignature('00'.repeat(64), 'x', 'abc')).rejects.toThrow(/64 hex/);
  });

  it('rejects a signature that is not 128 hex chars', async () => {
    const { publicKeyHex } = await generateKeypair();
    await expect(verifySignature('dead', 'x', publicKeyHex)).rejects.toThrow(/128 hex/);
  });
});
```
...
export async function generateKeypair(): Promise<Keypair> {
  ...
  return { privateKeyHex: bytesToHex(secret), publicKeyHex: bytesToHex(publicKey) };
}
...
<script>
  ...
  window.addEventListener('toast', (e) => {
    if (!region) return;
    const { message, type = 'success' } = (e as CustomEvent<ToastDetail>).detail;
...
<button
  ...
  class="inline-flex h-9 w-9 items-center justify-center rounded-full border border-hairline bg-canvas text-ink transition-colors hover:bg-canvas-soft-2"
>
  ...
  <svg class="hidden h-4 w-4 dark:block" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
    <circle cx="12" cy="12" r="4" /><path stroke-linecap="round" d="M12 2v2M12 20v2M4 12H2M22 12h-2M5 5l1.5 1.5M17.5 17.5 19 19M19 5l-1.5 1.5M6.5 17.5 5 19" />
  </svg>
</button>
...
<html lang="en">
  ...
  <body class="min-h-screen bg-canvas-soft text-ink">
    <Nav />
    <main>
      <slot />
    </main>
...
const { variant = 'primary', href, type = 'button', class: cls = '', id } = Astro.props;
const base =
...
---
{href ? (
  <a href={href} class={klass} id={id}><slot /></a>
) : (
  <button type={type} class={klass} id={id}><slot /></button>
)}
```
...
```astro
---
interface Props { elevation?: 1 | 2 | 3 | 4; class?: string; id?: string; }
const { elevation = 3, class: cls = '', id } = Astro.props;
const shadow = { 1: 'shadow-[var(--shadow-e1)]', 2: 'shadow-[var(--shadow-e2)]', 3: 'shadow-[var(--shadow-e3)]', 4: 'shadow-[var(--shadow-e4)]' }[elevation];
---
...
---
<span class={`inline-flex items-center rounded-full bg-canvas-soft px-2 py-0.5 font-mono text-xs text-body ${cls}`}><slot /></span>
```
...
const { class: cls = '' } = Astro.props;
---
<div aria-hidden="true" class={`mesh-gradient pointer-events-none absolute inset-0 -z-10 opacity-60 blur-3xl ${cls}`}></div>
```
...
```astro
---
interface Props { slot?: string; minHeight?: number; class?: string; }
const { slot, minHeight = 280, class: cls = '' } = Astro.props;
const adsenseId = import.meta.env.PUBLIC_ADSENSE_ID;
---
<div class={`mx-auto w-full max-w-3xl ${cls}`} style={`min-height:${minHeight}px`}>
  {adsenseId ? (
    <>
      <ins
    ...
    <div class="flex h-full min-h-[inherit] items-center justify-center rounded-md border border-dashed border-hairline text-xs text-mute">
      Advertisement
    </div>
  )}
</div>
...
```astro
---
interface Props { name: 'key' | 'sign' | 'verify' | 'shield' | 'arrow'; class?: string; }
...
---
<header class="sticky top-0 z-40 border-b border-hairline bg-canvas/80 backdrop-blur">
  <nav class="mx-auto flex h-16 max-w-[1200px] items-center justify-between gap-4 px-4 sm:px-6">
    <a href="/" class="font-mono text-sm font-medium text-ink">{SITE.name}</a>

    <ul class="hidden items-center gap-1 md:flex">
      {NAV_LINKS.map((l) => (
        <li>
          <a href={l.href} class="rounded-full px-3 py-2 text-sm text-body transition-colors hover:bg-canvas-soft-2 hover:text-ink">{l.label}</a>
        </li>
      ))}
    </ul>
    ...
    <div class="flex items-center gap-2">
      <ThemeToggle />
      <div class="hidden md:block"><Button href="/#tool" variant="primary" class="h-9 px-4 text-sm">Open tool</Button></div>
      <button id="nav-open" type="button" aria-label="Open menu" aria-expanded="false" aria-controls="mobile-menu" class="inline-flex h-9 w-9 items-center justify-center rounded-full border border-hairline md:hidden">
  ...
  <div id="mobile-menu" hidden class="fixed inset-0 z-50 bg-canvas md:hidden">
    <div class="flex h-16 items-center justify-between px-4">
      <span class="font-mono text-sm font-medium">{SITE.name}</span>
      <button id="nav-close" type="button" aria-label="Close menu" class="inline-flex h-9 w-9 items-center justify-center rounded-full border border-hairline">
    ...
    <ul class="flex flex-col gap-1 px-4 pt-4">
      {NAV_LINKS.map((l) => (
        <li><a href={l.href} class="block rounded-md px-3 py-3 text-lg text-ink hover:bg-canvas-soft-2">{l.label}</a></li>
      ))}
    </ul>
  </div>
</header>
...
<footer class="border-t border-hairline bg-canvas">
  <div class="mx-auto grid max-w-[1200px] gap-10 px-4 py-16 sm:px-6 md:grid-cols-[1.5fr_repeat(3,1fr)]">
    <div>
      <p class="font-mono text-sm font-medium text-ink">{SITE.name}</p>
      <p class="mt-3 max-w-xs text-sm text-body">{SITE.description}</p>
    </div>
    {FOOTER_SECTIONS.map((section) => (
      <div>
        <p class="font-mono text-xs uppercase tracking-wide text-mute">{section.title}</p>
        <ul class="mt-4 flex flex-col gap-2">
          {section.links.map((l) => (
            <li><a href={l.href} class="text-sm text-body transition-colors hover:text-ink">{l.label}</a></li>
          ))}
        </ul>
      </div>
    ))}
  </div>
  <div class="mx-auto flex max-w-[1200px] flex-col items-center justify-between gap-3 border-t border-hairline px-4 py-6 sm:flex-row sm:px-6">
    <p class="text-xs text-mute">© {year} {SITE.name}. All client-side. Keys never leave your browser.</p>
    <ul class="flex gap-4">
      {SOCIAL_LINKS.map((l) => (
        <li><a href={l.href} class="text-xs text-body hover:text-ink" rel="noopener">{l.label}</a></li>
      ))}
    </ul>
  </div>
</footer>
...
<Card id="keygen" class="scroll-mt-24">
  <div class="flex items-center gap-2">
    <Icon name="key" class="h-5 w-5 text-ink" />
    <h3 class="display-md">Generate a keypair.</h3>
  </div>
  <p class="mt-2 text-sm text-body">Creates a cryptographically secure Ed25519 keypair in your browser. The private key never leaves this page.</p>

  <Button id="keygen-btn" variant="primary" class="mt-5">Generate keypair</Button>

  <div class="mt-6 space-y-4">
    <div>
      <label class="font-mono text-xs uppercase tracking-wide text-mute">Public key · 64 hex</label>
      <div class="mt-1 flex items-start gap-2 rounded-md bg-ink p-3 font-mono text-xs text-on-primary">
        <output id="keygen-public" class="block w-full break-all">—</output>
        <button data-copy="#keygen-public" class="shrink-0 text-on-primary/70 hover:text-on-primary" aria-label="Copy public key">copy</button>
      </div>
    </div>
    <div>
      <label class="font-mono text-xs uppercase tracking-wide text-mute">Private key · 64 hex · keep secret</label>
      <div class="mt-1 flex items-start gap-2 rounded-md bg-ink p-3 font-mono text-xs text-on-primary">
        <output id="keygen-private" class="block w-full break-all">—</output>
        <button data-copy="#keygen-private" class="shrink-0 text-on-primary/70 hover:text-on-primary" aria-label="Copy private key">copy</button>
      </div>
    </div>
  </div>
</Card>
...
<script>
  ...
  const btn = document.getElementById('keygen-btn') as HTMLButtonElement | null;
...
<Card id="sign" class="scroll-mt-24">
  <div class="flex items-center gap-2">
    <Icon name="sign" class="h-5 w-5 text-ink" />
    <h3 class="display-md">Sign a message.</h3>
  </div>
  <p class="mt-2 text-sm text-body">Produce an EdDSA signature with your private key.</p>

  <div class="mt-5 space-y-4">
    <div>
      <label for="sign-message" class="font-mono text-xs uppercase tracking-wide text-mute">Message</label>
      <textarea id="sign-message" rows="4" class="mt-1 w-full rounded-sm border border-hairline bg-canvas p-3 font-mono text-sm text-ink focus:border-hairline-strong focus:outline-none" placeholder="Type a message to sign…"></textarea>
    </div>
    <div>
      <label for="sign-private" class="font-mono text-xs uppercase tracking-wide text-mute">Private key · 64 hex</label>
      <input id="sign-private" type="text" class="mt-1 w-full rounded-sm border border-hairline bg-canvas p-3 font-mono text-xs text-ink focus:border-hairline-strong focus:outline-none" placeholder="Paste your 64-hex private key…" />
    </div>
  </div>
  ...
  <div class="mt-6">
    <label class="font-mono text-xs uppercase tracking-wide text-mute">Signature · 128 hex</label>
    <div class="mt-1 flex items-start gap-2 rounded-md bg-ink p-3 font-mono text-xs text-on-primary">
      <output id="sign-output" class="block w-full break-all">—</output>
      <button data-copy="#sign-output" class="shrink-0 text-on-primary/70 hover:text-on-primary" aria-label="Copy signature">copy</button>
    </div>
  </div>
</Card>
...
<script>
  ...
  const msg = document.getElementById('sign-message') as HTMLTextAreaElement | null;
  const priv = document.getElementById('sign-private') as HTMLInputElement | null;
  const out = document.getElementById('sign-output');

  btn?.addEventListener('click', async () => {
    if (!msg?.value.trim()) { toast('Enter a message', 'error'); return; }
    if (!priv?.value.trim()) { toast('Enter a private key', 'error'); return; }
    btn.disabled = true;
...
<Card id="verify" class="scroll-mt-24">
  <div class="flex items-center gap-2">
    <Icon name="verify" class="h-5 w-5 text-ink" />
    <h3 class="display-md">Verify a signature.</h3>
  </div>
  <p class="mt-2 text-sm text-body">Check a signature against the original message and a public key.</p>

  <div class="mt-5 space-y-4">
    <div>
      <label for="verify-message" class="font-mono text-xs uppercase tracking-wide text-mute">Original message</label>
      <textarea id="verify-message" rows="4" class="mt-1 w-full rounded-sm border border-hairline bg-canvas p-3 font-mono text-sm text-ink focus:border-hairline-strong focus:outline-none" placeholder="The exact message that was signed…"></textarea>
    </div>
    <div>
      <label for="verify-public" class="font-mono text-xs uppercase tracking-wide text-mute">Public key · 64 hex</label>
      <input id="verify-public" type="text" class="mt-1 w-full rounded-sm border border-hairline bg-canvas p-3 font-mono text-xs text-ink focus:border-hairline-strong focus:outline-none" placeholder="Paste the 64-hex public key…" />
    </div>
    <div>
      <label for="verify-signature" class="font-mono text-xs uppercase tracking-wide text-mute">Signature · 128 hex</label>
      <textarea id="verify-signature" rows="2" class="mt-1 w-full rounded-sm border border-hairline bg-canvas p-3 font-mono text-xs text-ink focus:border-hairline-strong focus:outline-none" placeholder="Paste the 128-hex signature…"></textarea>
    </div>
  </div>

  <Button id="verify-btn" variant="primary" class="mt-5">Verify signature</Button>

  <div id="verify-result" class="mt-6 rounded-md border border-hairline bg-canvas-soft p-4 font-mono text-sm text-mute">Result will appear here…</div>
</Card>
...
<script>
  ...
  const msg = document.getElementById('verify-message') as HTMLTextAreaElement | null;
  const pub = document.getElementById('verify-public') as HTMLInputElement | null;
  const sig = document.getElementById('verify-signature') as HTMLTextAreaElement | null;
  ...
  btn?.addEventListener('click', async () => {
    ...
    try {
      const ok = await verifySignature(sig.value.trim(), msg.value, pub.value.trim());
      if (result) {
        result.textContent = ok ? '✓ VALID — signature matches.' : '✗ INVALID — signature does not match.';
        result.className =
...
import Icon from '../ui/Icon.astro';
---
<section id="tool" class="mx-auto max-w-[1200px] scroll-mt-20 px-4 py-16 sm:px-6">
  <div class="mx-auto max-w-2xl text-center">
    <Badge>RFC 8032 · EdDSA</Badge>
    <h2 class="display-lg mt-4">The complete Ed25519 workflow.</h2>
    <p class="mt-3 inline-flex items-center gap-2 text-sm text-body">
      <Icon name="shield" class="h-4 w-4" /> Everything runs in your browser. Keys never leave your device.
    </p>
  </div>
...
<section class="relative overflow-hidden">
  ...
  <div class="mx-auto max-w-[1200px] px-4 py-24 text-center sm:px-6 md:py-32">
    ...
    <h1 class="display-xl mx-auto mt-5 max-w-3xl text-balance">Generate, sign, and verify Ed25519 keys.</h1>
    <p class="mx-auto mt-5 max-w-xl text-lg text-body">A fast, private toolkit for modern digital signatures. RFC 8032 EdDSA, computed entirely in your browser — your private keys never touch a server.</p>
...
import Icon from '../ui/Icon.astro';
const features = [
  { icon: 'shield', title: 'Private by design.', body: 'All cryptography runs locally via WebCrypto. No keys, messages, or signatures are ever transmitted.' },
  { icon: 'key', title: 'Modern & fast.', body: 'Ed25519 is a state-of-the-art elliptic-curve signature scheme — small keys, fast verification, strong security.' },
  { icon: 'verify', title: 'Standards-based.', body: 'A faithful RFC 8032 EdDSA implementation built on the audited @noble/ed25519 library.' },
] as const;
---
<section class="mx-auto max-w-[1200px] px-4 py-16 sm:px-6">
  <div class="grid gap-6 md:grid-cols-3">
    {features.map((f) => (
      <Card elevation={2}>
        <Icon name={f.icon} class="h-6 w-6 text-ink" />
        <h3 class="display-md mt-4">{f.title}</h3>
        <p class="mt-2 text-sm text-body">{f.body}</p>
      </Card>
    ))}
  </div>
</section>
...
import Button from '../ui/Button.astro';
const faqs = [
  { q: 'Are my keys sent anywhere?', a: 'No. Key generation, signing, and verification run entirely in your browser using the WebCrypto API. Nothing is uploaded.' },
  { q: 'What is Ed25519?', a: 'Ed25519 is an EdDSA signature scheme using Curve25519. It offers high security with small (32-byte) keys and fast verification.' },
  { q: 'What are the key and signature sizes?', a: 'Private and public keys are 32 bytes (64 hex characters). Signatures are 64 bytes (128 hex characters).' },
] as const;
---
<section class="mx-auto max-w-3xl px-4 py-16 sm:px-6">
  <h2 class="display-lg text-center">Questions, answered.</h2>
  <div class="mt-8 divide-y divide-hairline rounded-lg border border-hairline bg-canvas">
    {faqs.map((f) => (
      <details class="group p-5">
        <summary class="flex cursor-pointer list-none items-center justify-between font-medium text-ink">
          {f.q}
          <span class="text-mute transition-transform group-open:rotate-45">+</span>
        </summary>
        <p class="mt-3 text-sm text-body">{f.a}</p>
      </details>
    ))}
  </div>
  <div class="mt-6 text-center"><Button href="/faq" variant="secondary">See all FAQs</Button></div>
</section>
```
...
<section class="bg-ink text-on-primary">
  <div class="mx-auto max-w-[1200px] px-4 py-20 text-center sm:px-6">
    ...
    <p class="mx-auto mt-3 max-w-lg text-base text-on-primary/70">Generate a keypair and sign your first message in seconds — no signup, no server.</p>
    <div class="mt-8"><Button href="/#tool" variant="secondary">Open the tool</Button></div>
  </div>
</section>
```
...
<BaseLayout title="Page not found" path="/404" noindex={true}>
  <section class="mx-auto flex max-w-2xl flex-col items-center px-4 py-32 text-center sm:px-6">
    <p class="font-mono text-sm text-mute">404</p>
    <h1 class="display-xl mt-4">This page doesn’t exist.</h1>
    <p class="mt-4 max-w-md text-body">The link may be broken or the page may have moved. Let’s get you back to signing.</p>
